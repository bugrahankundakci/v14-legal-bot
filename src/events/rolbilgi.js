...
module.exports = {
    ...
    async execute(interaction) {
        const minikinyetkilirolleri = interaction.member.roles.cache.some(r => minik.rol.yonetici.includes(r.id));
        if (!minikinyetkilirolleri) {
            return await interaction.reply({ content: `Bu komutu kullanmak için ${minik.rol.yonetici} rollerinden birine sahip olmanız gerekiyor.`, ephemeral: true});
        }
